...
function searchUsingAttribute()
{
	...
	$.ajax({url: "getFilmAttribute", data: data,success: function(text) { if(data.type == "json") { printJson(text, resultRegion2)
	}	else if (data.type == "text") { insertText(text, resultRegion2);}
		else if(data.type == "xml") { printXml(text, resultRegion2)}}, datatype: data.type});
}
